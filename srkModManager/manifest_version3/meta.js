...
Meta={ready:0,
    init:function(){
        ...
        Meta.send({req:'get mods', callback:'Meta.loadMods(Game.launchMods)'});
    },
    saveMods:function(){
        //save mod order
    ...
    loadMods:function(callback){
        ...
        for (let i=0;i<Meta.modList.length;i++){
            ...
            if (!found) {
                mod.disabled=true;
                Meta.send({req:'add mod', mod:{name:mod.name, disabled:mod.disabled}});
    ...
    newModPopup:function(){
        ...
        AddEvent(l('promptOption0'),'click',()=>{
            ...
            Meta.send({req:'add mod', mod:{name:l('nameMod').value, url:l('urlMod').value}});
            Game.LoadMod(l('urlMod').value);
            Game.ClosePrompt();
        });
    },
    send:function(data) {
        window.postMessage({sender:'Meta', data:data}, window);
    },
...
window.addEventListener("message", (event) => {
    ...
    if (event.data.sender=='MM') {
        Meta.request(event.data.data);
    }
});
